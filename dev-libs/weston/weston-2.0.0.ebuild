...
IUSE="colord dbus +drm editor examples fbdev +gles2 headless ivi jpeg +launch lcms rdp +resize-optimization screen-sharing static-libs +suid systemd test unwind wayland-compositor webp +X xwayland nvidia"
...
src_prepare() {
	if use nvidia; then
		eapply ${FILESDIR}/eglstreams/*.patch || die
	fi
...
src_configure() {
	...
	myconf+=" --with-cairo=image --enable-libinput-backend --enable-demo-client-install"
