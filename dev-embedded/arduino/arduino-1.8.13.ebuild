...
CORE="avr-1.8.3"
UPDATERV="0.10.10"
UPDATER="WiFi101-Updater-ArduinoIDE-Plugin-${UPDATERV}.zip"

SRC_URI="https://github.com/arduino/Arduino/archive/${PV}.tar.gz -> ${P}.tar.gz
	https://downloads.arduino.cc/cores/${CORE}.tar.bz2 -> ${PN}-${CORE}.tar.bz2
	https://github.com/arduino-libraries/WiFi101-FirmwareUpdater-Plugin/releases/download/v${UPDATERV}/${UPDATER} -> ${PN}-${UPDATER}
	doc? (
		${ARDUINO_DOCS_URI}
	)"
...
src_unpack() {
	...
	cp "${DISTDIR}/${PN}-${CORE}.tar.bz2" "${S}/build/${CORE}.tar.bz2" || die
	cp "${DISTDIR}/${PN}-${UPDATER}" "${S}/build/shared/${UPDATER}"|| die
