...
SRC_URI="
	amd64? ( https://developer.nvidia.com/vulkan-beta-${PV//.}-linux -> NVIDIA-Linux-x86_64-${PV}.run )
	tools? (
		https://download.nvidia.com/XFree86/nvidia-settings/nvidia-settings-${SETTINGS_VER}.tar.bz2
	)
"
...
src_compile() {
	...
	if use tools; then
		emake -C "${S}"/nvidia-settings-${SETTINGS_VER}/src/libXNVCtrl \
			DO_STRIP= \
			LIBDIR="$(get_libdir)" \
			NVLD="$(tc-getLD)" \
			NV_VERBOSE=1 \
		...
		emake -C "${S}"/nvidia-settings-${SETTINGS_VER}/src \
			DO_STRIP= \
...
src_install() {
	...
	if use tools; then
		...
		if use static-libs; then
			dolib.a "${S}"/nvidia-settings-${SETTINGS_VER}/src/libXNVCtrl/libXNVCtrl.a

			insinto /usr/include/NVCtrl
			doins "${S}"/nvidia-settings-${SETTINGS_VER}/src/libXNVCtrl/*.h
		fi
