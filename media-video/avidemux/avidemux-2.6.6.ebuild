...
IUSE="debug opengl nls qt4 gtk sdl vaapi vdpau video_cards_fglrx xv"
KEYWORDS="~amd64 ~x86"
...
"
RDEPEND="$DEPEND"
PDEPEND="~media-libs/avidemux-plugins-${PV}:${SLOT}[opengl?,qt4?,gtk?]"
...
src_install() {
	...
	if use gtk ; then
		fperms +x /usr/bin/avidemux3_gtk
		domenu ${PN}-2.6.desktop
	fi
}
